...
try {
    ...
    $queryCheckCuenta = "SELECT COUNT(*) FROM tn1_cuentas WHERE id = :cuenta_id";
    ...
    $queryCheckSetfile = "SELECT COUNT(*) FROM tn1_setfiles WHERE id = :setfile_id";
    ...
    foreach ($data['operaciones'] as $operacion) {
        try {
            ...
            // 🔹 **Paso 1: Buscar el ID del activo en `tn1_activos`**
            $queryActivo = "SELECT id FROM tn1_activos WHERE simbolo = :simbolo LIMIT 1";
            $stmt = $pdo->prepare($queryActivo);
            $stmt->execute([':simbolo' => $operacion['activo']]);
            $idActivo = $stmt->fetchColumn();

            // 🔹 **Si el activo no existe, lo creamos en `tn1_activos`**
            if (!$idActivo) {
                $queryInsertActivo = "INSERT INTO tn1_activos (simbolo) VALUES (:simbolo)";
            ...
            $queryCheckOperacion = "SELECT COUNT(*) FROM tn1_operaciones WHERE cuenta_id = :cuenta_id AND ticket = :ticket";
            ...
            $queryInsert = "INSERT INTO tn1_operaciones 

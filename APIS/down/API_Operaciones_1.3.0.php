...
try {
    ...
    // 🔹 Consultar operaciones en la BD con la nueva relación de `cuenta_id` 🔹
    if ($id_usuario) {
        $query = "SELECT o.id, o.cuenta_id, o.activo_id, o.setfile_id, o.tipo, o.volumen, 
                         o.precio_entrada, o.precio_salida, o.ganancia, o.fecha_apertura, o.fecha_cierre,
                         o.ticket, o.comision, o.swap, o.orden_id, o.posicion_id, o.comentario
                  FROM wp8e_operaciones o
                  INNER JOIN wp8e_cuentas c ON o.cuenta_id = c.id
    ...
    // 🔹 Calcular ganancia del último mes 🔹
    $primer_dia_mes = date('Y-m-01'); // Ejemplo: "2025-02-01"
    $ganancia_ultimo_mes = 0.00;

    foreach ($operaciones as $op) {
        if ($op['fecha_cierre'] >= $primer_dia_mes) {
            $ganancia_ultimo_mes += floatval($op['ganancia']);
        }
    }

    // 🔹 Calcular rentabilidad último mes 🔹
    $rentabilidad_ultimo_mes = ($portafolio_general > 0) ? ($ganancia_ultimo_mes / $portafolio_general) * 100 : 0.00;
    ...
    $rentabilidad_general = number_format(($portafolio_general > 0) ? ($ganancia_total / $portafolio_general) * 100 : 0.00, 2, '.', '');
    $ganancia_ultimo_mes = number_format($ganancia_ultimo_mes, 2, '.', '');
    $rentabilidad_ultimo_mes = number_format($rentabilidad_ultimo_mes, 2, '.', '');
    ...
    echo json_encode([
        ...
        'ganancia_ultimo_mes' => $ganancia_ultimo_mes,
        'rentabilidad_ultimo_mes' => $rentabilidad_ultimo_mes,
